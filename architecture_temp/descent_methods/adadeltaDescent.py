...
class AdadeltaDescent(FirstOrderDescentMethod):
    ...
    def __init__(self, initial_point, objective_function: Callable, objective_function_args: dict = {},
                 ...
                 use_auto_differentiation: bool = True, delta=0.01, gamma=0.9, tolerance=0.0001, max_function_calls=50):
    ...
    def optimize(self):
        ...
        dx = np.zeros(len(self.initial_point))
        ...
        while self.max_function_calls > self._function_calls and np.linalg.norm(x_curr - x_prev) > self.tolerance:
            ...
            update = - ((e + np.sqrt(dx)) / (e + np.sqrt(s))) * gradient
            dx = self.delta * dx + (1 - self.delta) * (update * update)
            x_curr = x_prev + dx
